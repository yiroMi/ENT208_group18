...
An IR emitter shines a beam toward a receiver. When a bin is full, waste blocks the beam. The receiver's output changes, and the STM32 detects this via GPIO.
...
When a valid voice command is received (and the bin is not full), the STM32 sends a PWM signal to the SG90 servo, which rotates the bin lid open (90°), then closes it after 3 seconds.
...
The flame sensor detects a lighter or candle flame (760–1100nm wavelength) and outputs a digital signal to the STM32 via GPIO, which then triggers the buzzer and LED alarm.
...
When the flame sensor detects a fire, the STM32 sends GPIO signals to turn on a buzzer (audible alarm) and a bright LED (visual alarm).
...
Each bin has a red/green LED. The STM32 reads the infrared beam sensor status via GPIO, then drives the corresponding LED: green when the bin is not full, red (flashing) when the bin is full. Workers can see at a glance which bin needs emptying during walkthroughs.
...
The STM32 communicates with the OLED screen via I2C to display the bin type ("Recyclable", "Kitchen waste", etc.) and example waste names ("Milk tea bottle", "Banana peel"), helping residents sort correctly.
...
Each bin has a tactile push button connected to a GPIO pin on the STM32. When a worker presses the button, the STM32 detects the level change. If the bin is not full, it drives the servo to open the lid (same logic as voice control). If the bin is full, the button has no effect.
...
- **Capacitive touch buttons** — no mechanical wear, but require dedicated touch detection circuitry and are sensitive to humidity (wet hands may cause false triggers or fail to trigger).
...
| Overflow detection | IR beam sensor | Non-contact, environment-proof |
